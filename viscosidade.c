...
void accel(float m,float kb,int n,float eps24,float eps,float rcutsq,
		...
		float sigsq, float boxlx, float boxly, float boxlz, float *V, float *W, float *Wxx, float *Wxy, float *Wxz, float *Wyz, float *Wzz);
...
int main(){
	...
	int atom = 12; //partícula a ser utilizada
	n = 1372;  // número de partículas
	...
	for(i=0;i<n;i++){		
	fscanf(in,"%d %f %f %f %f %f %f",&atom,&rx[i],&ry[i],&rz[i],&vx[i],&vy[i],&vz[i]); //o caractere & serve para salvar o valor da variável a cada rodada
...
fclose(in);
	...
	int NC = cbrt(n*0.25); //número de células
	printf("nc = %d\n",NC);
	...
	accel(m,kb,n,eps24,eps,rcutsq,rx,ry,rz,ax,ay,az,sigsq,boxlx,boxly,boxlz,&V,&W,&Wxx,&Wxy,&Wxz,&Wyy,&Wyz,&Wzz);
	...
	kin = 0.5*m*sumv2*1e7/(float)(n); //J/molécula 
	float H = kin+V;
	
	T = 2.0*kin/(3.0*r);
	
	float Vol = a*NC*a*NC*a*NC*1e-30; //m³
	float Pr = ((float)(n)/NA*r*T/Vol + W/3*eps24*m*1e-3/Vol/NA)*1e-5; //bar
	
	float PIii = (Wxx+Wyy+Wzz + vxx+vyy+vzz)*eps24*m/Vol/3 + W/3*eps24*m*1e-3/Vol/NA;
  
	printf("%f %f %f %f %f %f %f\n",t,T,Pr,PIii,kin,V,H);
	//fprintf(temp,"%f %f %f %f %f %f %f \n",t,T,Pr,PIii,kin,V,H);
			...
			rijsq = rxij*rxij+ryij*ryij+rzij*rzij;
				...
				if(rijsq<=rcutsq){ 
					...
					*V = *V+pot;
					...
					az[j]= az[j]-aij*rzij;	
					*W = *W + wij;
					*Wxx = *Wxx + aij*rxij*rxij;
					*Wxy = *Wxy + aij*rxij*ryij;
					*Wxz = *Wxz + aij*rxij*rzij;
					*Wyy = *Wyy + aij*ryij*ryij;
					*Wyz = *Wyz + aij*ryij*rzij;
					*Wzz = *Wzz + aij*rzij*rzij;
